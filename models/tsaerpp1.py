...
from models.spiking_er.losses import CELoss, KLLoss
from models.spiking_er.soft_dtw import SoftDTW


class Tsaerpp1(ContinualModel):
    ...
    def __init__(self, backbone, loss, args, transform, dataset=None):
        ...
        """
        super(Tsaerpp1, self).__init__(backbone, loss, args, transform, dataset=dataset)
        ...
        self.ce_loss = CELoss()
        self.tskl_loss = KLLoss(tau=self.tau)
    ...
    def observe(self, inputs, labels, not_aug_inputs, epoch=None):
        ...
        if not self.buffer.is_empty():
            ...
            # The buffer logits are transposed to the shape [T, B, K]
            buf_logits = buf_logits.transpose(0, 1).contiguous()

            # The model processes the buffer inputs, buf_outputs.shape: [T, B, K]
            buf_outputs = self.net(buf_inputs)

            # Computes the TSKL loss, and applies the scaling factors
            loss_kl_raw = self.kl_loss(t_logits=buf_logits, s_logits=buf_outputs)
            loss_kl = self.alpha * (self.tau ** 2) * loss_kl_raw
            ...
            # Retrieves a batch of inputs and their labels from the buffer
            ...
            # The model processes the buffer inputs, buf_out2.shape: [T, B, K]
            buf_out2 = self.net(buf_x2)

            # Computes the TSCE loss over the buffer data, and applies the scaling factor
            loss_ce_buf_raw = self.ce_loss(s_logits=buf_out2, targets=buf_y2)
            loss_ce_buf = self.args.theta * loss_ce_buf_raw

            # Computes the total loss
            loss = loss_ce_raw + loss_kl + loss_sdtw + loss_ce_buf
        else:
            loss = loss_ce_raw
