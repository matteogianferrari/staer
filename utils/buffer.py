...
class SdtwBuffer:
    ...
    examples: torch.Tensor  # (mandatory) buffer attribute: the tensor of images
    labels: torch.Tensor  # (optional) buffer attribute: the tensor of labels
    logits1: torch.Tensor  # (optional) buffer attribute: the tensor of logits with 1 shape
    logits2: torch.Tensor  # (optional) buffer attribute: the tensor of logits with another shape
    logits3: torch.Tensor  # (optional) buffer attribute: the tensor of logits with a different shape
    task_labels: torch.Tensor  # (optional) buffer attribute: the tensor of task labels
    true_labels: torch.Tensor  # (optional) buffer attribute: the tensor of true labels
    ...
    def __init__(self, buffer_size: int, device="cpu", sample_selection_strategy='reservoir', **kwargs):
        ...
        self.device = device
        self.num_seen_examples = 0
        self.attributes = ['examples', 'labels', 'logits1', 'logits2', 'logits3', 'task_labels', 'true_labels']
        self.attention_maps = [None] * buffer_size
        self.sample_selection_strategy = sample_selection_strategy
    ...
    def init_tensors(self, examples: torch.Tensor, labels: torch.Tensor,
                     logits1: torch.Tensor, logits2: torch.Tensor, logits3: torch.Tensor, task_labels: torch.Tensor,
                     true_labels: torch.Tensor) -> None:
        """
        Initializes just the required tensors.
        ...
        Args:
            examples: tensor containing the images
            labels: tensor containing the labels
            logits1: tensor containing the outputs of the network with 1 shape
            logits2: tensor containing the outputs of the network with another shape
            logits3: tensor containing the outputs of the network with another another shape
            task_labels: tensor containing the task labels
            true_labels: tensor containing the true labels (used only for logging)
        """
        for attr_str in self.attributes:
    ...
    def add_data(self, examples, labels=None, logits1=None, logits2=None, logits3=None, task_labels=None, attention_maps=None, true_labels=None, sample_selection_scores=None):
        """
        Adds the data to the memory buffer according to the reservoir strategy.
        ...
        Args:
            examples: tensor containing the images
            labels: tensor containing the labels
            logits1: tensor containing the outputs of the network
            logits2: tensor containing the outputs of the network
            logits3: tensor containing the outputs of the network
            task_labels: tensor containing the task labels
            attention_maps: list of tensors containing the attention maps
            true_labels: if setting is noisy, the true labels associated with the examples. **Used only for logging.**
            sample_selection_scores: tensor containing the scores used for the sample selection strategy. NOTE: this is only used if the sample selection strategy defines the `update` method.
        ...
        """
        if not hasattr(self, 'examples'):
            self.init_tensors(examples, labels, logits1, logits2, logits3, task_labels, true_labels)
        ...
        for i in range(examples.shape[0]):
            ...
            if index >= 0:
                ...
                if logits1 is not None:
                    self.logits1[index] = logits1[i].to(self.device)
